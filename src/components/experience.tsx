...
export function Experience() {
  ...
  // Scroll Spy State
  const [activeState, setActiveState] = useState<{ active: Set<string>, neighbors: Set<string> }>({ active: new Set(), neighbors: new Set() });
  ...
  useEffect(() => {
    const handleScroll = () => {
        ...
        items.forEach((item) => {
            ...
            if (distanceY < activationThreshold) {
                newActive.add(item.key);
            } else if (distanceY < neighborThreshold) {
                newNeighbors.add(item.key);
            }
        });

        setActiveState({ active: newActive, neighbors: newNeighbors });
    };

    // Throttle or use requestAnimationFrame for performance
    let ticking = false;
    const onScroll = () => {
        if (!ticking) {
    ...
    window.addEventListener('resize', onScroll); // Re-calc on resize
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 gap-y-12">
            {items.map((item, index) => {
                ...
                const isActive = activeState.active.has(item.key);
                const isNeighbor = activeState.neighbors.has(item.key);

                return (
                    <motion.div 
                        key={item.key}
                        ref={el => { if(el) itemRefs.current.set(item.key, el); }}
                        data-key={item.key}
                        ...
                        className={`
                            ...
                            ${isActive 
                                ? "shadow-2xl translate-y-[-8px] scale-[1.03] bg-blue-50/40 dark:bg-blue-900/20 border-blue-500/60 dark:border-blue-400/60 z-30 ring-4 ring-blue-500/10 dark:ring-blue-400/10" 
                                : isNeighbor
                                    ? "shadow-lg translate-y-[-4px] scale-[1.01] bg-white dark:bg-gray-900 border-blue-200 dark:border-blue-800 z-20"
                                    : "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-lg hover:translate-y-[-2px] hover:border-gray-300 dark:hover:border-gray-700 hover:z-10"
                            }
                        `}
                    >
